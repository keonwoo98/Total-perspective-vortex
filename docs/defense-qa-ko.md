...
**❓ Q&A**
- *'검증 도구'는 어디?* — 정확히: "validation tools"는 *채점 방법*이지 데이터가 아님. `predict`는 `train`이 저장한 **held-out 검증셋**(처음 보는 문제)으로 예측하고 **정답 대비 정확도**를 계산 — 이게 검증. `cross_val_score`를 *다시 부르진 않음*(그건 `train`의 일); 여기선 held-out 정확도가 곧 검증.
...
**🖥️ 시연:** `python mybci.py 4 14 predict` — 모든 줄 끝에 추론 지연시간이 붙어 나옴, 예: `epoch 00:  [2]  [1] False  0.42 ms`. 이벤트당 ~0.1–0.4ms로 2초 예산보다 한참 아래.

**❓ Q&A**
- *진짜 라이브 스트림?* — 사전 녹음에 대한 이벤트 단위 시뮬레이션; 처리 모델(한 번에 하나, 2초 예산)은 라이브와 동일.
- *왜 빠른가?* — 반복 계산이 없음.
- *2초의 대부분은?* — 2초 창 모으기; 계산은 ~0.5ms.
- *2초는 어디서 보장 / 지연시간 어떻게 증명?* — `predict`가 추론마다 `time.perf_counter`로 재서 **지연시간 컬럼으로 출력**(~0.1–0.4ms); `assert latency < LATENCY_BUDGET_S`(2.0초)가 어떤 이벤트라도 예산 넘으면 크래시; 게다가 에폭 자체가 이벤트 후 2초 창.
...
**📚 알아야 할 개념 — 밑바닥부터 차근히 (프로젝트의 핵심, `csp.py`):**

*문제부터:*
- **왜 한 문제에 ~2만 개 숫자가 나오나.** 한 문제 = **64채널 × 321시간 = 20,544개 숫자**(마이크 64개가 2초 동안 각자 321번 측정). 이 개수가 "차원".
- **왜 줄여야 하나 = 차원 축소.** 한 사람당 문제가 ~45개뿐인데 숫자가 20,544개면 *너무* 많아 배울 수가 없어요 — 분류기가 그냥 **외워버려서**(과적합) 새 문제엔 실패. 게다가 다 필요 없어요: ERD에서 봤듯 답은 **출렁임이 얼마나 큰가**에 있지 모든 값에 있지 않아요. **차원 축소** = 그 ~2만 개를 판별 정보만 남긴 몇 개로 압축 — 여기선 **4개**.
- **CSP가 뭔가.** **CSP(Common Spatial Patterns)**가 제가 구현한 축소기 — 두 클래스가 출렁임 크기에서 제일 다르게 갈리는 **채널 조합**을 찾아줘요.

*20,544 → 4가 어떻게 되나 (한 문제를 **표: 세로 64줄=채널 × 가로 321칸=시간 = 20,544칸**으로 보기):*
- **압축 1 — 채널 64 → 4.** **레시피 1개** = 채널마다 가중치 64개 `w`. 그걸로 64채널을 섞으면 **새 "가상 채널" 하나** `z = wᵀE`(321칸짜리 줄) — DJ가 64트랙을 한 곡으로 섞듯. **레시피 1개 → 줄 1개; 레시피 4개 → 4줄** (`64×321 → 4×321`). 각 레시피는 **감지기**: 한 동작엔 크게, 다른 동작엔 작게 출렁 — 보통 **2개 왼손, 2개 오른손.**
- **압축 2 — 시간 321 → 1.** 각 가상채널을 **얼마나 출렁였나** = **분산** `var(z)`으로 줄이고, 정규화(줄 합으로 나눔) 후 **log**. `4×321 → 4`. **합쳐서 20,544 → 4.**

*CSP가 그 4개 레시피를 어떻게 찾나 (수학).* 각 클래스의 **공분산** `C = (E·Eᵀ)의 평균`(64×64, trace 정규화 — "채널들이 같이 출렁이는 정도"). 그러면 레시피가 *그 클래스에서* 내는 분산은 `wᵀC w`. 한 클래스엔 크고 다른 클래스엔 작은 레시피를 원하니 — **비율**을 최대화:
> `λ = wᵀC₁w / wᵀ(C₁+C₂)w`  = 그 레시피 출렁임 중 클래스1이 차지한 비율.
이 비율을 최대화하는 것이 곧 **일반화 고유문제 `C₁w = λ(C₁+C₂)w`**의 해 = `eigh(C1, C1+C2)`의 **고유벡터**들(각자 **고유값 λ**). λ→1=클래스1 전용 감지기, λ→0=클래스2, **λ=0.5=쓸모없음**; **λ가 0.5에서 가장 먼 고유벡터 4개**를 챙김. (공분산·고유분해엔 numpy/scipy 허용 — 나머지는 직접 구현.)

**📚 분류기 — LDA (4숫자가 들어가는 곳).** 한 문제의 4숫자 = **점 하나**(~45문제 → ~45점), **두 무리(구름)**를 이룸. LDA가 그 사이에 **직선 울타리 하나**를 그음:
- 각 무리의 **중심** `μ₀, μ₁` = 클래스별 점들의 평균; **퍼짐** `Σ` = 점들의 4×4 공분산.
- 울타리 **방향**: `w = Σ⁻¹(μ₁ − μ₀)` — 중심→중심 화살표 `μ₁−μ₀`를 `Σ⁻¹`로 재보정(넓게 퍼진 노이즈 방향은 덜 믿음).
- 울타리 **위치**: `b = −wᵀ(μ₀+μ₁)/2` (중간점 + 클래스 균형 보정).
- 새 점 `x`는 **`w·x + b`의 부호**로 판정: 양수 → 한쪽, 음수 → 다른쪽 = 울타리 어느 쪽인지.

(LDA는 *차원 축소가 아님* — 이 항목은 축소기 CSP를 채점. 자작 LDA는 보너스 2.11-D.)

**🗣️ 말로 설명하기 — CSP (구어체; 개념 이름은 말하고, 진짜 수식은 괄호 안):** "CSP는 차원을 줄이는 겁니다. 한 문제가 64채널 곱하기 321시간이라 2만 개가 넘는 숫자인데, 한 사람당 문제가 45개밖에 안 돼서 이걸 그대로 넣으면 모델이 그냥 외워버려요. 그래서 CSP로 4개로 줄이는데, 두 단계예요. 첫째, 채널을 64개에서 4개로: '레시피'를 만드는데 채널마다 가중치를 하나씩 준 64개짜리 묶음이고, 그걸로 64채널을 곱해서 다 더하면(`z = wᵀE`) 새 '가상 채널' 하나가 나와요 — DJ가 64트랙을 한 곡으로 섞듯이. 레시피 4개면 채널이 4개죠. 둘째, 시간을 321에서 1로: 각 가상채널이 얼마나 크게 출렁였는지, 그 분산(`var(z)`)만 재서 로그를 씌우면 4숫자가 남아요. 그리고 그 레시피 4개는 두 동작이 출렁임 크기에서 제일 다르게 갈리도록 골라요 — 각 동작의 공분산 표를 만들고, 어떤 방향에서 한 동작의 출렁임이 전체 중 차지하는 비율(`λ = wᵀC₁w / wᵀ(C₁+C₂)w`)이 제일 극단적인 방향을 찾는 거예요. 1이나 0에 가까우면 한쪽 전용 감지기, 0.5면 쓸모없고요. 이 방향 찾는 게 고유분해이고 — 고유값이 그 비율 λ, 고유벡터가 레시피 — 그 고유분해까지 직접 구현했습니다. 라이브러리 CSP는 안 쓰고, numpy랑 scipy의 `eigh`만 썼는데 그건 서브젝트가 허용한 거예요."

**🗣️ 말로 설명하기 — LDA (구어체):** "그 4숫자는 4차원 공간의 점 하나예요. 한 문제가 점 하나라 45문제면 두 무리(구름)가 생기죠, 동작별로. LDA는 그 사이에 선 하나를 그어요. 두 무리 각각의 중심 — 그냥 점들의 평균(`μ₀, μ₁`) — 을 구하고, 한 중심에서 다른 중심으로 가는 방향을 무리가 퍼진 모양, 그러니까 공분산으로 보정해서 정합니다(`w = Σ⁻¹(μ₁−μ₀)`) — 노이즈 많은 방향은 덜 믿게요. 울타리는 두 중심의 가운데쯤에 놓고요(`b`). 새 점이 오면 그 울타리 어느 쪽에 있는지로 판별해요(`w·x + b`의 부호)."
...
- **내가 만든 것: FBCSP(Filter-Bank CSP).** **필수 파이프라인과 같은 PhysioNet 데이터**로 돌아요 — **CSP 단계만 바뀌고** 데이터·LDA는 그대로(새 데이터셋 아님). 기본 CSP는 7–30Hz 전체를 **한 덩어리**로 봐요. FBCSP는 **7–30Hz를 4개 좁은 하위 밴드(8-12, 12-16, 16-20, 20-30Hz)로 쪼개** 밴드마다 **따로 CSP**를 돌리고 특징을 이어붙여요.
