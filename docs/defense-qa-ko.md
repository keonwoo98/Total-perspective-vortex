...
- **압축 1 — 채널 64 → 4.** **레시피 1개** = 채널마다 가중치 하나씩 64개. 그걸로 64채널을 섞으면(곱해서 더하기, `@`) **새 "가상 채널" 하나**(321칸짜리 줄)가 나와요 — DJ가 64트랙을 한 곡으로 섞듯. **레시피 1개 → 줄 1개; 레시피 4개 → 4줄** (`64×321 → 4×321`). 그리고 각 레시피는 **감지기**예요: 그 가상채널이 한 동작엔 크게, 다른 동작엔 작게 출렁여요 — 보통 **2개는 왼손, 2개는 오른손 감지기.**
- **압축 2 — 시간 321 → 1.** 각 가상채널이 **얼마나 크게 출렁였나** = 그 줄의 **분산**을 재서 숫자 1개; 그다음 정규화하고 **log**. 즉 `4×321 → 4`. **합쳐서 20,544 → 4** (레시피 1개가 64채널을 1줄로, 분산 1개가 321칸을 1숫자로 먹음).

*CSP가 그 4개 레시피를 어떻게 찾나:* 두 동작이 출렁임 크기에서 **제일 다르게** 갈리는 섞는 법 4개, 즉 "**제일 한쪽으로 쏠린 감지기 4개**"를 골라요. (전문적으론 이게 `eigh(C1, C1+C2)`의 상위 **고유벡터**들이고, 각자 **λ**(얼마나 한쪽으로 쏠렸나)로 점수 매겨 0.5에서 가장 먼 4개를 챙김. 공분산·고유분해엔 numpy/scipy 허용 — 나머지는 직접 구현.)

**📚 4숫자가 들어가는 곳 — LDA(분류기):** 한 문제의 4숫자 = **점 하나**(~45문제 → 점 ~45개), 이게 **두 무리(구름)**를 이룸(클래스별로). **LDA는 그 두 무리 사이에 직선(울타리) 하나를 긋고**, 새 점은 **그 선의 어느 쪽에 떨어지나**로 분류. (수식 `w = Σ⁻¹(μ1−μ0)` — 중심→중심 방향을 구름 퍼짐으로 보정 — 은 보너스 2.11-D.) LDA는 *차원 축소가 아님* — 이 항목은 축소기(CSP)만 다룸.
