...
*20,544 → 4가 실제로 어떻게 되나.* 한 문제를 **표(세로 64줄=채널 × 가로 321칸=시간 = 20,544칸)**로 보세요. 두 번의 압축으로 줄어요:
- **압축 1 — 줄(채널) 수 줄이기 (채널 64 → 4).** **레시피 1개** = 가중치 64개. 매 시간칸에서 64채널 값을 레시피대로 곱해 더해(`가중치 · 칸`) **그 칸 값 1개**; 321칸 다 하면 → **새 줄 1개**(가상 채널). 즉 **레시피 1개 → 새 줄 1개**, **레시피 4개 → 4줄**. 표가 `64×321 → 4×321`(20,544 → 1,284); 칸 수는 그대로.
- **압축 2 — 각 줄 → 숫자 1개 (시간 321 → 1).** 한 줄 = 321개 출렁임 값; 그 줄을 **분산**으로 요약하면 "얼마나 크게 출렁였나" 숫자 1개, 그다음 **log**. 즉 `4×321 → 4`. **합쳐서 20,544 → 4** (레시피 1개가 64채널을 1줄로, 분산 1개가 321칸을 1숫자로 먹음).

*왜 하필 그 4개 레시피냐 (핵심):*
- 아무거나 안 골라요 — 두 클래스가 출렁임 크기에서 **제일 다르게** 갈리는 걸 골라요. 각 클래스의 **공분산**(채널들이 같이 출렁이는 64×64 표)을 만들고, **일반화 고유분해 `eigh(C1, C1+C2)`**가 레시피들을 점수 **λ = a/(a+b)**(그 레시피 출렁임 중 클래스1이 차지한 비율)로 순위 매겨 돌려줘요. **λ가 1이나 0에 가까우면 좋은 감지기; λ=0.5면 쓸모없음.** **λ가 0.5에서 가장 먼 4개**를 보관. (공분산·고유분해엔 numpy/scipy 허용 — 나머지는 직접 구현.)

**📚 4숫자가 들어가는 곳 — LDA(분류기):** 한 문제의 4숫자 = **4차원 점 하나**; A클래스 문제들은 한 점 무리(구름), B클래스는 다른 구름. **LDA는 그 두 구름 사이에 직선(울타리)을 긋고**, 새 점은 **어느 쪽에 떨어지나**로 분류. (수식 `w = Σ⁻¹(μ1−μ0)` — 중심→중심 방향을 구름 퍼짐으로 보정 — 은 보너스 2.11-D.) LDA는 *차원 축소가 아님* — 이 항목은 축소기(CSP)만 다룸.
