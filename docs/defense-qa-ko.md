...
**❓ Q&A**
- *그래프가 두 종류인데 왜 둘 다 보여주나?* — 같은 신호를 두 방식으로 본 것. **시계열**(시간축)은 매끈해지는 걸 *눈으로* 보게 하고, **PSD**(주파수축)는 어느 주파수가 남았는지를 *증명*. 필터링은 하나, 그 결과를 두 각도로.
- *필터가 실제로 한 일은?* — 원본은 ~0~80Hz가 섞여 있음(80 = 160Hz 샘플링의 절반 = Nyquist 한계). 밴드패스가 **7~30Hz만 남기고 7 아래·30 위를 거의 0으로** 누름.
- *필터 전/후 둘 다 0~80인데 왜 다르게 보이나?* — x축은 일부러 같음; **안에 든 내용**만 다름(전/후). 0~80 전체를 보여줘야 "대역 밖이 제거됨"이 보임.
- *PSD의 y축은?* — 세기(그 주파수가 얼마나 강한지), 단위 **dB**(로그). 높을수록 강함; x축은 주파수(Hz).
- *왜 음수 dB가 있나?* — dB는 **기준(1 µV², 라벨의 `re 1 µV²`) 대비**(로비 기준 층수처럼). 음수 = "기준보다 약함"이지 음수 세기가 아님. 로그라 아주 센 것과 약한 것을 한 그래프에 같이 봄.
- *왜 하필 7~30Hz?* — 운동상상 **뮤(8~12)·베타(13~30)** 리듬(ERD)이 거기 삶; 평가표도 ~8~40 요구. 7 아래=느린 드리프트, 30 위=근육/전기 잡음.
- *필터 종류는?* — **FIR**(firwin 설계), **zero-phase**라 타이밍 안 밂. 경계는 수직 절벽이 아니라 가파른 비탈(유한 FIR).
- *"average reference"가 정확히 뭔가?* — 매 순간 **64채널 평균을 각 채널에서 빼서** 모든 전극에 공통인 성분을 제거(EEG 전압은 항상 어떤 기준 대비로 측정됨).
- *그 평균엔 각 채널 고유 신호도 들어가지 않나?* — 들어감. 하지만 **공통은 평균에 살아남고 고유(부호 제각각)는 대체로 상쇄**돼서 평균≈공통. 빼면 공통 잡음 제거, 각 채널 손실은 ~1/64(≈1.5%)뿐 — 남는 장사, 채널 많을수록 더 좋음.
- *"기준"이 두 개 — 헷갈리지 말 것:* **average reference**는 신호를 바꾸는 진짜 전처리(`preprocessing.py`); **1 µV² 기준**은 PSD의 dB 눈금일 뿐(MNE 그림 규칙, `viz.py`, 전처리 아님).
- *PSD는 어디서 계산?* — `preprocessing.py`가 아니라 `viz.py`가 MNE의 `compute_psd(method="welch").plot()` 호출. PSD/dB는 **시각화**일 뿐 모델에 안 들어감 — 파이프라인은 필터된 *시계열* → 에폭 → CSP를 씀.
- *'영상'은?* — 주제·제출물에 영상 없음; 핵심은 "필터된 게 깨끗"이고 PSD가 명확히 보임.
- *색칠 색은?* — MNE 자동; 파랑=T0, 주황=T1, 초록=T2(범례 확인, 색 자체에 의미 없음).
