...
*20,544 → 4가 실제로 어떻게 되나 — 두 번의 압축:*
- **압축 1 — 64채널을 4개로 섞기 (채널 64 → 4).** **공간 필터("레시피")** = 채널마다 가중치 하나씩 64개. 적용하면(행렬곱 `@` = "곱해서 더하기") 64채널을 **새 "가상 채널" 하나**로 블렌딩 — DJ가 64트랙을 볼륨 손잡이로 한 곡으로 섞듯. 레시피 4개로: `64×321 →(@)→ 4×321`. (레시피 ①의 큰 가중치는 오른쪽 운동영역에 — "오른쪽 운동 감지기".)
- **압축 2 — 시간을 크기 하나로 접기 (시간 321 → 1).** 각 가상 채널의 **분산**(=출렁임이 얼마나 큰가)을 재서 → 숫자 1개, 그다음 **log**. 즉 `4×321 → 4`. **합쳐서 20,544 → 4.**
