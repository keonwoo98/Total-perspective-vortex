...
**📚 알아야 할 개념:**
- **EEG = 머리에 붙인 마이크 64개.** 64개 전극이 각자 **초당 160번** 전압을 µV로 잼. 한 녹음 = `(64, 20000)` 표 (~125초 × 160Hz).
- **채널 이름 = 머리 위치.** `C`=정수리/**운동영역**; 홀수=왼쪽, 짝수=오른쪽. **C3**(왼쪽 운동) ↔ 오른손, **C4** ↔ 왼손 — 뇌는 **반대쪽** 제어.
- **ERD — 이게 가능한 이유.** *쉬는* 운동영역은 8–30Hz로 크게 출렁임(신경세포가 장단 맞춰 박수치듯). *쓰면*(움직이거나 상상하면) 박자가 깨져 **출렁임이 줄어듦** = **ERD**, **반대쪽**에서(오른손 → C3 조용). 실측(20명 평균 µV²): 왼손 C3 83.1 / C4 **78.7**; 오른손 C3 **82.2** / C4 89.3 — 진짜지만 미묘. **답은 출렁임의 *크기*(분산)에 있지 파형 모양이 아님.**
- **필터(진짜 전처리).** ① **평균 기준**: 매 순간 **64채널 평균을 각 채널에서 빼** 공통 성분 제거. ② **7–30Hz 밴드패스(FIR, firwin, zero-phase)**: ERD가 사는 뮤(8–12)·베타(13–30)만 남기고 느린 드리프트(<7)·근육/전기잡음(>30) 버림. 긴 신호를 먼저 필터, 그다음 에폭.
- **에폭.** 데이터에 표시 **T0(휴식), T1/T2(두 동작)**가 박혀 있음. 휴식 버리고 T1/T2마다 **2초 창**을 오려냄 → 한 문제 = `(64 × 321)`, **321 = 2초 × 160 + 1**(양 끝 포함). 한 사람·한 실험 ≈ **45문제**.
...
**📚 알아야 할 개념:**
- **특징(feature)** = 분류기에 주는 압축적·판별적 숫자(raw 신호가 아님).
- 운동상상의 **의미 있는 대역 = 뮤(8–12) + 베타(13–30Hz)** — ERD가 사는 곳. 아무 대역(예 50–60Hz)을 남기면 신호는 있어도 무의미.
- 우리 특징 = **CSP 로그-분산**: 문제당 4숫자 = 판별 공간패턴에서의 ERD 세기(항목 2.7에서 만듦).

**🗣️ 할 말:** "핵심 체크는 *의미 있는* 대역을 남겼나입니다 — 7–30Hz가 뮤·베타, 바로 운동상상 신호(ERD)가 사는 곳을 보존(PSD에서 확인). 64×321을 4개 판별 특징으로 압축하는 건 CSP라 Implementation에서 다루고, '어느 주파수가 중요한지 학습'은 FBCSP 보너스예요."
**🖥️ 시연:** `FMIN = 7.0, FMAX = 30.0`(`config.py`) + 필터 후 PSD.

**❓ Q&A**
- *왜 30까지만, 40 아니고?* — ERD가 8–30에서 제일 강하고 30 위는 잡음↑; 8–30 표준, 0.658로 검증.
- *분류기에 들어가는 특징은?* — raw 필터 신호(~2만/문제)가 아니라 **CSP 로그-분산** — 4숫자 = 판별 패턴의 ERD 세기.
- *필터된 신호 자체가 이미 특징 아닌가?* — 필터는 *대역*만 남김; 특징은 압축적·판별적이어야. CSP가 ~2만을 의미 있는 4로.
- *왜 로그-분산?* — 분산=출렁임 크기(ERD); log가 종 모양으로 펴고 곱셈을 덧셈 차이로 → 직선 분류기에 유리.
...
**📚 알아야 할 개념 — LDA (그 4숫자를 받는 분류기):** 4숫자 = 4차원 점, 두 클래스 구름. **`w = Σ⁻¹(μ1 − μ0)`** = 중심→중심 방향을 퍼짐 `Σ`로 보정; 판정 = **`w·x + b`** 부호. (LDA 자체는 보너스 2.11-D로 직접 구현.)
...
- *왜 exp0(왼손vs오른손)이 제일 어렵나(~0.57)?* — 둘 다 손, 인접·대칭 운동영역이라 반대쪽 차이가 작음; 양손vs양발은 훨씬 쉬움.
