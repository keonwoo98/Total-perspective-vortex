...
*How 20,544 → 4 actually happens.* Think of a trial as a **table: 64 rows (channels) × 321 columns (time) = 20,544 cells.** Two cuts shrink it:
- **Cut 1 — fewer rows (channels 64 → 4).** One **recipe** = 64 weights. At each time column it collapses the 64 channel-values into one blended value (`weights · column`); doing that across all 321 columns yields **one new row** (a "virtual channel") — like a DJ mixing 64 tracks into one. So **one recipe → one row**, and **4 recipes → 4 rows**. The table goes `64×321 → 4×321` (20,544 → 1,284); columns unchanged.
- **Cut 2 — each row → one number (time 321 → 1).** A row is 321 wiggle-values; its **variance** summarizes the whole row into one number ("how big the oscillation is"), then **log**. So `4×321 → 4`. **Total: 20,544 → 4** (one recipe eats 64 channels → 1 row; one variance eats 321 samples → 1 number).
...
**📚 What the 4 numbers feed — LDA (the classifier):** each trial's 4 numbers are a **point in 4-D**; class-A trials form one cloud of points, class-B another. **LDA draws a straight boundary (a "fence") between the two clouds**, and classifies a new point by **which side it lands on**. (Its math `w = Σ⁻¹(μ1−μ0)` — the center-to-center direction, corrected by the clouds' spread — is the from-scratch bonus, item 2.11-D.) LDA is *not* dimensionality reduction — this item is only about the reducer (CSP).
