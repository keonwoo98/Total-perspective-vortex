...
**🗣️ Say it out loud — CSP (spoken; concept names spoken, formulas kept in parentheses):** "CSP reduces the dimension. One trial is 64 channels times 321 time samples, so over 20,000 numbers — but a subject only has about 45 trials, so if I fed all of that in, the model would just memorize. So CSP squeezes it down to 4 numbers, in two steps. First, channels 64 to 4: I build 'recipes', each a set of 64 weights, one per channel, and I blend all 64 channels with a recipe — multiply each channel by its weight and add them up (`z = wᵀE`) — to make one new 'virtual channel', like a DJ mixing 64 tracks into one; four recipes give four channels. Second, time 321 to 1: for each virtual channel I just measure how big it wiggled — that's the variance (`var(z)`) — and take a log, which leaves 4 numbers. And I choose those 4 recipes so the two classes differ most in wiggle size: I build a covariance matrix for each class, and I look for the direction where the ratio of one class's variance to the total (`λ = wᵀC₁w / wᵀ(C₁+C₂)w`) is most extreme — near 1 or 0 means a great one-sided detector, 0.5 means useless. Finding those directions is an eigendecomposition — the eigenvalue is that ratio λ, the eigenvector is the recipe — and I implemented the eigendecomposition myself; no library CSP, only numpy and scipy's `eigh`, which the subject allows."

**🗣️ Say it out loud — LDA (spoken):** "The 4 numbers are a point in 4-D space; each trial is one point, so about 45 trials make two clouds — one per class. LDA draws a single fence between them. I take each cloud's center — just the mean of its points (`μ₀, μ₁`) — and the direction from one center to the other, corrected by how spread out the clouds are using the within-class covariance, so noisy directions count for less (`w = Σ⁻¹(μ₁−μ₀)`); the fence sits about halfway between the two centers (`b`). Then a new point is classified by which side of the fence it lands on — the sign of `w·x + b`."
