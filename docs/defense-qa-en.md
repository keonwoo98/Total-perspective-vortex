...
**🗣️ Say:** "Let me start with preprocessing — this is where the raw recording becomes something usable. I wrote a single script that loads a subject's EEG and shows it in **two complementary views, each before and after filtering**. First the **time series** — the raw wavy signal next to the 7–30 Hz filtered one — so you can see the cleanup with your own eyes. Then the **PSD (power spectral density)**, which is basically an equalizer view: how much energy sits at each frequency. The two views make the same point from different angles — that the filtered signal is genuinely *cleaner*, because we strip the slow drift and the line noise and keep only the motor-rhythm band (mu/beta) that actually carries the answer."
